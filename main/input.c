...
static void callback(button_t *btn, button_state_t state)
{
    bus_send_event(event_types[state], &btn->ctx, sizeof(btn->ctx));
}
...
esp_err_t input_init()
{
    ...
    buttons[INPUT_BTN_MAIN].ctx = (void *)INPUT_BTN_MAIN;
    ...
    buttons[INPUT_BTN_RESET].ctx = (void *)INPUT_BTN_RESET;
    ...
    buttons[INPUT_BTN_UP].ctx = (void *)INPUT_BTN_UP;
    ...
    buttons[INPUT_BTN_DOWN].ctx = (void *)INPUT_BTN_DOWN;
